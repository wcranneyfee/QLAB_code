...
def find_T1(filepath, fignum):
    ...
    fit = curve_fit(exponentialFunc, df['Delay Time (ms)']/1000, df['Voltage (V)'],
                    check_finite=True)

    popt, pcov = fit[0], fit[1]

    x_arr = np.linspace((df['Delay Time (ms)']/1000).min(), (df['Delay Time (ms)']/1000).max(), 1000)
    y_arr = exponentialFunc(x_arr, *popt)
    plt.plot(x_arr, y_arr)
    plt.scatter((df['Delay Time (ms)']/1000).values, df['Voltage (V)'].values, color='red')
...
def find_T2(filepath, fignum, cutoff=25000, argrelmax_order=1200, min_peak_height=1, windowlength_divisor=1000, min_time=0, drop_factor=2, polyorder=3):
